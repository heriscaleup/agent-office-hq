...
test('authenticated Nadia API persists analysis and proposed tasks across restart', async () => {
  ...
  try {
    ...
    assert.equal(adsStatus.provider.source, 'google_ads_via_omnirank');
    assert.equal(adsStatus.provider.status, 'UNAVAILABLE');
    assert.doesNotMatch(JSON.stringify(adsStatus), /shared.?secret|refresh.?token|developer.?token/i);
